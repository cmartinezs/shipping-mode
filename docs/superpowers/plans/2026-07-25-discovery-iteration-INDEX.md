...
|---|------|--------------------------|--------|-----|
| 1 | Scan foundation | A (goals/non-goals), B (fingerprint algorithm, sources catalog schema), C (scope `commands` schema), D.1 (`discover scan`, read-only) | In progress — 15 tasks, adversarial review round 1 fixes applied | `2026-07-25-discovery-scan-foundation.md` |
| 2 | `DiscoveryProposal` and validation | D.2 (proposal contract), D.3 steps 1–4 (structural/semantic validation, live re-verification, reference resolution, drift reconciliation) | Not started | *(to be written)* |
